...
class RealFakeDataset(Dataset):
    def __init__(self, opt):
        ...
        if '2b' not in opt.arch:
            ...
            self.transform = transforms.Compose([
                self.cut if opt.isTrain else rz_func,
                data_aug_func,
                crop_func,
                flip_func,
                transforms.ToTensor(),
    ...
    def cut(self, imgs):
        transform = transforms.Compose([
            ...
            transforms.FiveCrop(224),  # 五次裁剪
            transforms.Lambda(lambda crops: self.random_crops(crops))
            # transforms.Lambda(lambda crops: transforms.ToTensor()(self.random_crops(crops)))
        ])
