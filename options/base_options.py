...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--blur_sig', default='0.0, 2.0')       # sigma的变化范围
        parser.add_argument('--jpg_prob', type=float, default=0.5)
        parser.add_argument('--jpg_method', default='cv2,pil')
        parser.add_argument('--jpg_qual', default='30, 100')        # quality范围
        ...
        parser.add_argument('--class_bal', default=None) # 平衡采样每个类别
        parser.add_argument('--batch_size', type=int, default=256, help='input batch size')
        parser.add_argument('--loadSize', type=int, default=224, help='scale images to this size') # 224
        ...
        # 当serial_batches设置为True时，数据批次会按照顺序（通常是按照文件名或数据点在列表中的位置）被加载。
        # 这意味着每个epoch中的数据加载顺序是固定的，批次之间的数据不会随机打乱。
